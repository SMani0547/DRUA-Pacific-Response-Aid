...
## Refinement Phase — High-Fidelity Prototype

The dashboard was extended from a monitoring surface into an end-to-end **decide → approve → act** workflow. Six additions:

1. **Incident Intake Flow** — a "Submit report" dialog next to the CSV importer captures area, issue, severity, urgency, road access, resources, reporter type, and free-text description. On submit, the AI risk score is computed from those fields and the report drops straight into the live queue with status `New`.
2. **AI Response Action Plan** — the area side-panel now renders a numbered 3–5 step plan generated by the Gemini AI Agent, each step with an ETA badge, derived from the incident's severity, population, road access, and issue type.
3. **Human Approval Workflow** — every report has an explicit status (`New → Under Review → Approved → Dispatched → Resolved`). Approve / Send to Review / Mark Dispatched / Mark Resolved buttons live in the side panel; the priority table and command board have inline status pickers. AI recommends, a human always approves.
